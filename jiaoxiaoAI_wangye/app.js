...
renameBtn.addEventListener('click', () => {
	...
	if (!chat) return;
	const name = prompt('输入新的会话名', chat.name) || chat.name;
	chat.name = name.trim() || chat.name;
	save();
	renderHistory();
...
function appendMessageUI(role, content) {
	...
	row.className = 'msg';
	row.innerHTML = '<div class="avatar ' + (role === 'assistant' ? 'ai' : '') + '">' + (role === 'assistant' ? '交小AI' : '👤') + '</div><div class="bubble">' + renderMarkdown(content || '') + '</div>';
...
function setThinking(bubbleEl, on) {
	if (on) bubbleEl.innerHTML = '<span class="thinking">正在思考中…</span>';
}
